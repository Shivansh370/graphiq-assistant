...
from llm import embeddings_provider


examples = [
    {
        "question": "Which client is associated with a loan that has the Loan Number 8217589624?",
        "query": 
"""MATCH (l:Loan)-[:HAS_CLIENT]->(c:Client)
WHERE l.loanNumber = 8217589624
RETURN l.loanNumber as loanNumber, c.clientNumber as ClientNumber
""",
    },
    {
        "question": "Can you provide the details of all loans associated with InvestorB , including loan type and servicing status?",
        "query":
"""MATCH (l:Loan)-[:BELONGS_TO]->(i:Investor), (l)-[:IS_OF_TYPE]->(lt:LoanType)
WHERE i.investorName = 'InvestorB'
RETURN i.investorName as investorName, l.loanNumber as loanNumber, l.originationDate as originationDate, l.aquisitionDate as aquisitionDate, l.paidOffDate as paidoffDate, l.maturityDate as maturityDate, l.loanServicingStatus as loanServicingStatus, l.loanTrackingStatusKey as loanTrackingStatusKey, l.loanClassificationStatus as loanClassificationStatus,  l.loanEscrowStatus as loanEscrowStatus 
""",
    },
    {
        "question": "What loans are linked to an investor with name 'InvestorA' and what is their unpaid principal balance?",
        "query":
"""MATCH (l:Loan)-[:BELONGS_TO]->(i:Investor)
WHERE i.investorName = 'InvestorA'
RETURN l.loanNumber as loanNumberLinkedToInvestorA, l.unpaidPrincipleBalance as unpaidPrincipleBalance
""",
    },
    {
        "question": "What is the loan servicing status for the loan linked to client number 978?",
        "query":
"""MATCH (l:Loan)-[:HAS_CLIENT]->(c:Client)
WHERE c.clientNumber = 978
RETURN l.loanNumber as loanAssociatedWithClient978, l.loanServicingStatus as servicingStatus
""",
    },
    {
        "question": "What is the current tracking status of the loan 8209527981?",
        "query":
"""MATCH (l:Loan)
WHERE l.loanNumber = 8209527981
RETURN l.loanNumber as loanNumber, l.loanTrackingStatusKey as trackingStatus
""",
    },
    {
        "question": "Can you find loans that have both a 'Restricted Escrow Balance' and an 'Unpaid Principle Balance'?",
        "query":
"""MATCH (l:Loan)
WHERE l.restrictedEscrowBalance IS NOT NULL AND l.unpaidPrincipleBalance IS NOT NULL
RETURN l.loanNumber as loanNumber, l.restrictedEscrowBalance as restrictedEscrowBalance, l.unpaidPrincipleBalance as unpaidPrincpleBalance
""",
    },
    {
        "question": "When was the loan 8257133144 originated and when is its maturity date?",
        "query":
"""MATCH (l:Loan)
WHERE l.loanNumber = 8257133144 
RETURN l.loanNumber as loanNumber, l.originationDate as originationDate, l.maturityDate as maturityDate
""",
    },
    {
        "question": "Can you show me loans originated before '2020-01-01' and their associated investor?",
        "query":
"""MATCH (l:Loan)-[:BELONGS_TO]->(i:Investor)
WHERE l.originationDate < date('2020-01-01')
RETURN l.loanNumber as loanNumber, l.originationDate as originationDate, i.investorName as investor
ORDER BY originationDate DESC
""",
    },
    {
        "question": "Which loans will mature within the next 6 months?",
        "query":
"""MATCH (l:Loan)
WHERE date() < l.maturityDate <= date() + duration({{months: 6}})
RETURN date() as currentDate, l.loanNumber as loanNumber, l.maturityDate as maturityDate
ORDER BY maturityDate ASC
""",
    },
    {
        "question": "What loans have escrow balances above $3700and what are their loan types?",
        "query":
"""MATCH (l:Loan)-[:IS_OF_TYPE]->(t:LoanType)
WHERE l.escrowBalance > 3700 
RETURN l.loanNumber as loanNumber, l.escrowBalance as escrowBalance, t.loanTypeDescription as loanType
""",
    },
    {
        "question": "What is the unpaid principal balance of all loans associated with the investor Investor A?",
        "query":
"""MATCH (l:Loan)-[:BELONGS_TO]->(i:Investor)
WHERE i.investorName = 'InvestorA'
RETURN l.loanNumber as loanAssociatedWithInvestorA, l.unpaidPrincipleBalance as unpaidPrincipleBalance
""",
    },
    {
        "question": "Can you list all loans with their status, category, and origination date for the client 978?",
        "query":
"""MATCH (l:Loan)-[:HAS_CLIENT]->(c:Client), (l)-[:HAS_REO_CATEGORY]->(r:REOCategory)
WHERE c.clientNumber = 978
RETURN l.loanNumber as loanAssociatedWithClient978, l.originationDate as originationDate, l.loanTrackingStatusKey as trackingStatus, l.loanClassificationStatus as classificationStatus, l.loanEscrowStatus as escrowStatus, l.loanServicingStatus as servicingStatus, r.REOCategory as REOCategory
""",
    },
    {
        "question": "List all loans for clients with numbers above 970, including their status and unpaid principal balance.",
        "query":
"""MATCH (l:Loan)
WHERE l.loanNumber > 970
RETURN l.loanNumber as loanNumber, l.loanServicingStatus as servicingStatus, l.loanEscrowStatus as escrowStatus, l.loanClassificationStatus as classificationStatus, l.loanTrackingStatusKey as trackingStatus, l.unpaidPrincipleBalance as unpaidPrincipleBalance
""",
    },
    {
        "question": "What is the unpaid principal balance for loans linked to Investor A with loan types 'FT' or 'VA'?",
        "query":
"""MATCH (l:Loan)-[:BELONGS_TO]->(i:Investor), (l)-[:IS_OF_TYPE]->(t:LoanType)
WHERE i.investorName = 'InvestorA' AND t.loanTypeDescriptionId IN ['FT', 'VA']
RETURN l.loanNumber as loanNumber, t.loanTypeDescriptionId as typeDescriptionId, t.loanTypeDescription as typeDescription, l.unpaidPrincipleBalance as unpaidPrincipleBalance
""",
    },
    {
...
""",
    ...
    {
        "question": "Which are all the definition for the term 'Business Segment Id'?",
        "query":
"""MATCH (t:Term)-[:HAS_DEFINITION]-(d:Definition)
WHERE t.termName = 'Business Segment Id'
RETURN d.termDefinition
""",
    },
    {
...
example_selector = SemanticSimilarityExampleSelector.from_examples(
    examples, embeddings_provider, Neo4jVector, k=3, input_keys=["question"], **{"node_label": "Example"}
)



def select_similar_examples(question):
    NL = "\n"
    fewshot_examples = example_selector.select_examples({"question": question})
    if fewshot_examples is None:
        return "No similar examples found in the database"
    formatted_examples = (NL * 2).join(
        [
            f"Question: {el['question']}{NL}Cypher:{NL}{el['query']}"
